...
\begin{itemize}
    ...
    Quando a rede tem essa função de ativação, o neurônio é um modelo linear generalizado conhecido como regressão logística. Dessa forma, notamos que a função de ativação tem um papel semelhante à função de ligação: relacionar o preditor linear $v$ a um valor esperado $\mu$ de um dado $y$ \citep{sarle1994,frei2020,mccullagh1989}. % ficou meio jogado?
...
%One important characteristic is weight sharing that is some model parameters are identicala across time. This is related to the notion of stationarity.
