...
\subsection{Entropy | Cross-entropy | Perplexity (WIP)}
...
\textbf{Entropia} é uma medida de informação. Dada uma variável aleatória $X$ que abrange tudo o que estivermos prevendo (palavras, letras, tokens... um conjunto $\chi$) e uma particular função de probabilidade $p(x)$, a entropia da variável aleatória $X$ é:
\begin{equation}
    \label{entropia}
    H(X) = - \sum_{x \in \chi} \biggl(p(x) \; log_2 (p(x)) \biggl)
\end{equation}
O logaritmo ($log$), em princípio, pode ser computado em qualquer base. Se utilizamos $log$ na base 2, o valor resultante da entropia é mensurado em \textit{bits}. 
...
\begin{equation}
    H(w_1,\dots,w_n) = - \sum_{w_{1:n} \in L} \biggl( p(w_{1:n}) \; log(w_{1:n}) \biggl).
\end{equation}
Podemos definir, também, a taxa de entropia (ou entropia por palavra) da sequência dividindo pelo número de palavras:
\begin{equation}
    \frac{1}{n}H(w_{1:n}) = - \frac{1}{n} \sum_{w_{1:n} \in L} \biggl( p(w_{1:n}) \; log(w_{1:n}) \biggl).
\end{equation}
...
\begin{align}
    H(L) & = \lim_{n \rightarrow \infty} \; \frac{1}{n} H(w_1,\cdots,w_n) \\
         & = \lim_{n \rightarrow \infty} \; - \frac{1}{n} \sum_{W \in L} \biggl( p(w_1,\cdots,w_n) \; log(p(w_1,\cdots,w_n)) \biggl).
\end{align}
...
\begin{equation}
    H(p,m) = \lim_{n \rightarrow \infty} \; - \frac{1}{n} \sum_{W \in L} \biggl( p(w_1,\cdots,w_n) \; log(m(w_1,\cdots,w_n)) \biggl).
\end{equation}
...
\begin{equation}
    H(p,m) = \lim_{n \rightarrow \infty} \; - \frac{1}{n} \sum_{W \in L} \biggl( log(m(w_1,\cdots,w_n)) \biggl).
\end{equation}
...
\begin{align}
    Perplexidade(W) & = 2^{H(W)} \\
                    & = p(w_1,\cdots,w_n)^{-\frac{1}{n}}.
\end{align}
...
\begin{itemize}
    ...
    \item \textit{dense encoding (feature embeddings)}: cada categoria é embutida (\textit{embedded}) em um vetor $d \in \N$ dimensional.
\end{itemize}
