...
% resumo 
    % português
\begin{abstract}
\nico{não gostei do resultado que ficou, está mais longa e confusa}

O problema de composição musical automática é um problema já explorado extensivamente na literatura. Em geral, o objetivo final nesses trabalhos é a composição musical em si, de forma que os modelos utilizados são calibrados com o intuito de que a composição final gerada pelo modelo seja adequada em algum sentido.  Por sua vez, o impacto que pequenas modificação dos parâmetros do modelo responsável pelas composições ocasionam na composição final é um assunto que não é discutido na literatura. Neste trabalho, tempos por objetivo estudar a sensibilidade dos modelos de machine learning mais usados na composição automática de música no que tange pequenas modificações na arquitetura utilizada. A ferramente utilizada para este fim é uma métrica denominada perplexidade. Por fim as músicas são avaliadas ainda de forma subjetiva, no que tange a qualidade e musicalidade das composições obtidas com os métodos empregados.
\end{abstract}

    % inglês
\begin{englishabstract}
The issue of automatically composed music has received a lot of attention in the literature. The literature generally focuses on the composition itself, and models are tuned to produce useful results. Technical details like the effects that slight adjustments to the model's parameters have on the composition, however, are still largely unknown. In this work, we investigate how sensitive the most popular machine learning algorithms for music composition are to the consequences that even slight alterations in the composition's design can have. To accomplish this task, we apply a metric called perplexity. The composed works are then subjectively assessed for musicality and quality.
\end{englishabstract}
...
Uma função de ativação pode ser linear ou não linear. \nico{Seu objetivo em geral é transformar o valor de $v_k := u_k + b_k$ para um valor que esteja de acordo com a distribuição de $y_k$, como pode ser visto em \eqref{vk}}. A escolha de uma particular função de ativação depende do problema que o neurônio almeja resolver. \nico{Em problemas de classificação de classe binária, ela é responsável por levar o valor de $v_k \in \R$ para o intervalo $(0,1)$, possibilitando a predição das respectivas classes}. Abaixo, seguem alguns exemplos:
...
%De acordo com \citet{goodfellow2016}, as LSTM fazem parte de uma classe de modelos chamada de RNN fechadas (\textit{gated RNN}), que estão entre os modelos para dados sequenciais com maior sucesso em aplicações práticas. RNNs fechadas são baseadas na ideia de criar caminhos ao longo do tempo que contêm derivadas que não dissipam e nem explodem a cada passo temporal (\textit{each time step}), ou seja, . Além disso, RNNs fechadas permitem que a rede acumule informação (como a evidência de um atributo ou categoria particular) de uma longa duração. No entanto, uma vez que a informação foi utilizada, talvez seja útil que esse estado antigo seja descartado. Por exemplo, se uma sequência é feita de sub-sequências, e queremos acumular a evidência dentro de cada sub-sequência, precisamos de um mecanismo para descartar o estado antigo, reiniciando o mecanismo (\textit{setting it to zero}). As RNNs fechadas aprendem quando fazer isso.
...
\obs{
\begin{itemize}
    \item entropia cruzada: ``Quando a verdadeira distribuição...''
    \item seja $m$ um estimador da p.m.f. de $p$, quando a verdadeira entropia é desconhecida, pode-se utilizar a entropia cruzada para aproximar $p$ 
    \item trocar modelo por estimador
\end{itemize}
}
...
%\textbf{Entropia} é uma medida de informação. Seja $\chi$ um conjunto finito de \textit{tokens} e $X$ uma variável aleatória discreta associada à esses \textit{tokens} com função massa de probabilidade $p$. Define-se a entropia da variável aleatória $X$ como
%\begin{equation}
%    \label{entropia}
...
%\end{align*}
\pumi{Cara-crachá daqui pra diante}
...
\end{align*}
Medidas menores de perplexidade são indicativas de uma predição melhor. Por fim, é importante destacar que a perplexidade de dois modelos de linguagem apenas podem ser comparadas se ambos utilizam o mesmo vocabulário (\textit{corpus}) \citep{manning1999, kamath2019, jurafsky2021}.
...
\end{itemize}
\nico{Depois}, ambas as bases de dados utilizadas passaram por três procedimentos: tratamento, união e tokenização. Abaixo, alguns detalhes sobre cada uma:
\begin{itemize}
...
\begin{enumerate}
    \item \nico{construir um modelo com os devidos parâmetros (\textit{vocab\_size}, \textit{lstm\_units}, \textit{embedding\_dim} e \textit{batch\_size}), mas fixar $batch\_size=1$ para que haja apenas um vetor de entrada e um de saída}; 
    \item \nico{fixado o modelo, carregam-se os pesos de um modelo similar ajustado previamente, sendo a única diferença entre eles o parâmetro \textit{batch\_size}};
    \item \nico{o processo gerador envolve} fornecer uma sequência de caracteres inicial, neste trabalho foi fornecida ``X:'', pois é como qualquer música \nico{presente no arquivo} \textit{.abc} inicia;
    \item \nico{de forma iterativa}, o modelo estima um novo \nico{elemento} para compor a sequência até \nico{atingir} um \nico{comprimento} definido (no caso, utilizou-se 1000);
    \item via expressões regulares, são extraídas da sequência \nico{blocos de texto candidatos a músicas e, ao serem convertidos com sucesso, resultam em} músicas.
\end{enumerate}
...
\begin{table}
    \caption{Resultados dos experimentos referentes à base de dados Irish, com $vocab\_size=64$, utilizando a função \textit{tanh} como função de ativação.} 
...
\begin{itemize}
    ...
    \item \nico{O impacto do \textit{learning\_rate} na função perda é evidente, acusando um aumento na perda quando a taxa de aprendizagem é menor. Isso ocorre, pois, ao ser calculado o gradiente, o passo foi demasiado pequeno para o processo de otimização.} 
...
\end{itemize}
\nico{A figura \ref{fig:irish_tanh_corr} apresenta a matriz de correlação entre os parâmetros e as métricas, sendo o $\rho$ de Spearman a medida de correlação. É notável que a matriz corrobora os padrões identificados}. Destaque para a forte correlação negativa de $-0.87$ de \textit{learning\_rate} \nico{tanto com a perda quanto com a perplexidade}. Ademais, é interessante notar que \textit{batch\_size} apresentou uma (fraca) correlação positiva com a perplexidade, sendo um indicativo de tendência a sobreajuste do modelo. 
%
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{irish_tanh_vs_logit.pdf}
...
\end{figure}
    ...
    % ABC Notation
\section{\nico{ABC Notation}}
\nico{Por meio de uma abordagem similar a Irish, avaliou-se a primeira etapa para os casos de ABC Notation. A tabela \ref{table:abcnotation_tanh_full} contém os resultados dos 32 modelos ajustados e na figura \ref{fig:abcnotation_tanh_loss} constam as perdas calculadas ao longo das épocas dos respectivos ajustes}.
...
\rowcolors{2}{gray!25}{white}
\begin{table}
    \caption{Resultados dos experimentos referentes à base de dados ABC Notation, com $vocab\_size = 125$, utilizando a função \textit{tanh} como função de ativação.} \label{table:abcnotation_tanh_full}
    \centering
    \resizebox{\linewidth}{!}{
        \begin{tabular}[t]{ccccccccc}
        \toprule
...
\begin{itemize}
    \item \nico{No geral, não é claro o impacto de \textit{batch\_size} na perda, mas permanece a redução de variabilidade a cada época}.
    \item \nico{Diferentemente do observado em Irish, o aumento de \textit{seq\_length} apresentou, comumente, uma redução na função perda. Novamente, permanece a redução de variabilidade}.
    \item \nico{Assim como notado em Irish, \textit{learning\_rate} tem um impacto expressivo na função perda, manifestando um aumento na perda quando a taxa de aprendizagem é menor}. 
    \item \nico{O parâmetro \textit{embedding\_dim} continuou sem expor modificações significativas na função perda}.
    \item Permanece o comportamento demonstrado em Irish para \textit{lstm\_units}. 
\end{itemize}
\nico{A figura \ref{fig:abcnotation_tanh_corr} apresenta a matriz de correlação entre os parâmetros e as métricas, sendo o $\rho$ de Spearman a medida de correlação. Uma vez mais, a matriz corrobora os padrões identificados, assim como a forte correlação negativa de $-0.87$ de \textit{learning\_rate} com a perda e com a perplexidade}. Ademais, é interessante notar que \textit{batch\_size} apresentou uma (fraca) correlação negativa com a perplexidade, sendo o oposto observado na base Irish.
...
%\nico{
%\begin{itemize}
%    \item conversão de músicas (texto --> midi) + modelos
%    \item comentários sobre músicas geradas (geral e comparação entre modelos)
%    \item músicas estão disponibilizadas em bla bla
%\end{itemize}
%}


Para cada modelo selecionado na segunda etapa, considerando ambas as bases e ambas as funções de ativação, foram geradas sequências de texto até compor dez blocos candidatos a músicas. Após isso, utilizou-se Python para a conversão dos arquivos \textit{.abc} em música. Se a conversão é bem sucedida, uma nova peça musical é gerada; caso contrário, \pumi{o arquivo é invalidado. Isto ocorre pois como não foi artificialmente imposta uma estrutura que garanta que toda peça gerada é compatível com uma música, isto significa que o arquivo .abc composto não gera uma música.} Os resultados observados seguem:
\begin{itemize}
