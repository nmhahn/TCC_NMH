...
%% arquiteturas base e objetivo da rede neural

% Uma rede neural pode ser composta por três tipos de camadas: entrada, oculta e saída. 
    % A camada de entrada é um vetor de entradas numéricas (reais), podendo ser consideradas como as covariáveis de um modelo de regressão. 
    % A camada oculta, se existir, é o resultado após a combinação linear entre os sinais de entrada e os pesos, junto da função de ativação.
    % A camada de saída é o resultado das camadas ocultas (ou de entrada, caso a oculta não exista) após a combinação linear entre o resultado anterior e pesos, aplicados à função de ativação. Seu tamanho (nº de neurônios) pode variar dependendo do objetivo: regressão (1), classificação binária (2) ou classificação múltipla (n).
% As redes neurais são treinadas por meio 
